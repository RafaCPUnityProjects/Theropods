...
public class PlayerInput : MonoBehaviour
{
	...
	// Menu input override
	public string menuButtonInput;
	public float menuButtonValue;
	public SimulateInputType menuInput;
	...
	private void Update ()
	{
		...
		if (stateHandler && settingsManager && Time.time > 0f)
		{
			...
			if (settingsManager.inputMethod == InputMethod.KeyboardOrController || settingsManager.inputMethod == InputMethod.MouseAndKeyboard)
			{
				try
				{
			...
			else
			{
				...
				else if (settingsManager.inputMethod == InputMethod.MouseAndKeyboard || settingsManager.inputMethod == InputMethod.TouchScreen)
				{
					mousePosition = Input.mousePosition;
				}
	...
	public void SimulateInput (SimulateInputType input, string axis, float value)
	{
		if (axis != "")
		{
			...
			Invoke ("StopSimulatingInput", 0.01f);
		}
	}
	...
	private float InputGetAxisRaw (string axis)
	{
		...
		{
			StopCoroutine ("StopSimulatingInput");
			StopSimulatingInput ();
	...
	private bool InputGetButtonDown (string axis)
	{
		try
		{
